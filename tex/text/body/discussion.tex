...
Under configurations with guarantee static MRCA resolution at arbitrarily generational depth, memory footprint also scales with $G$ --- either $\mathcal{O}(G)$ or $\mathcal{O}(\log G)$ depending on whether guarantees are absolute or recency-proportional.  % TODO \citep{OTHERPREPRINT}
