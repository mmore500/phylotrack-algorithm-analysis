...
Under configurations with guarantee static MRCA resolution at arbitrarily generaitonal depth, memory footprint also scales with $G$ --- either $\mathcal{O}(G)$ or $\mathcal{O}(\log G)$ depending on whether guarantees are absolute or recency-proportional \citep{OTHERPREPRINT}.  % TODO
